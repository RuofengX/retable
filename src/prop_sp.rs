...
impl PropStorage for PropValueSp {
    fn append(&mut self, eid: EID, value: PropValue) -> () {
        ...
        if target_len > current_len {
            (current_len..(target_len - 1)).for_each(|_| {
                wtx.push(None);
            })
        }
...
mod tests {
    ...
    fn test_prop_value_sp() {
        ...
        // Insert values
        assert_eq!(
            prop_value_sp.append(EID(1), PropValue::Str("Value 0".to_string())),
            ()
        );
        assert_eq!(
            prop_value_sp.append(EID(2), PropValue::Str("Value 1".to_string())),
            ()
        );
        assert_eq!(
            prop_value_sp.append(EID(3), PropValue::Str("Value 2".to_string())),
            ()
        );
        assert_eq!(
            prop_value_sp.append(EID(5), PropValue::Str("Value 5".to_string())),
            ()
        );

        // Get values
        assert_eq!(
            prop_value_sp.get(EID(1)).map(|v| v.read().clone()),
            Some(PropValue::Str("Value 0".to_string()))
        );
        assert_eq!(
            prop_value_sp.get(EID(2)).map(|v| v.read().clone()),
            Some(PropValue::Str("Value 1".to_string()))
        );
        assert_eq!(
            prop_value_sp.get(EID(3)).map(|v| v.read().clone()),
            Some(PropValue::Str("Value 2".to_string()))
        );
        assert_eq!(
            prop_value_sp.get(EID(5)).map(|v| v.read().clone()),
            Some(PropValue::Str("Value 5".to_string()))
        );
